...
export const Store: React.FC<Props> = ({ user, settings }) => {
  ...
  const featuresList = tierType === 'BASIC' 
    ...
    : [
        ...
        'Unlimited access to AI Research',
    ...
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-300 pb-24 relative min-h-screen bg-black text-white font-sans">
      ...
      <div className="max-w-md mx-auto pt-8 px-4">
          ...
          <div className="text-center mb-8">
              ...
              <p className="text-slate-400 text-sm font-medium">Ask once, get trusted answers fast</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
              {subscriptionPlans.filter(p => p.name.includes('Monthly') || p.name.includes('Yearly')).map(plan => {
                  ...
                  const saving = isYearly ? (price * 0.2).toFixed(0) : null; // Mock saving calculation if not available
                      ...
                      >
                          ...
                          <div>
                              <div className="flex items-baseline gap-1">
                                  ...
                                  {!isYearly && <span className="text-xs text-slate-500">per month</span>}
                              </div>
                              {perMonth && (
                                  <p className="text-xs text-slate-500 mt-1">₹{perMonth.toLocaleString('en-IN')}/month</p>
                              )}
                          </div>
                      </button>
                  );
              ...
              {/* If no monthly/yearly found, show generic fallback */}
              {subscriptionPlans.length > 0 && !subscriptionPlans.some(p => p.name.includes('Monthly') || p.name.includes('Yearly')) && (
                  subscriptionPlans.map(plan => {
                      ...
                      return (
                        <button
                          key={plan.id}
                          onClick={() => setSelectedPlanId(plan.id)}
                          className={`p-5 rounded-2xl border text-left transition-all relative group flex flex-col justify-between h-24 ${
                              isSelected 
                                ? 'bg-[#18181b] border-cyan-500 shadow-[0_0_0_1px_rgba(6,182,212,1)]' 
                                : 'bg-[#0f0f11] border-slate-800 hover:border-slate-700'
                          }`}
                        >
                            <span className={`font-bold text-sm ${isSelected ? 'text-cyan-400' : 'text-slate-200'}`}>{plan.name}</span>
